...
const index = (props) => {
    ...
    const _updateProduct = async (id = '', name = '', brand = '', price = '', description = '', category = '', countInStock = '', rating = '', numReviews = '', image = '', isFeatured = '') => {
        if (name.length == 0) {
            name = item.name;
        ...
        if (image == undefined) {
            await updateProduct(null, id, name, description, brand, price, category, countInStock, rating, numReviews, isFeatured).then((data) => {
                console.log(data)
            })
        } else {
            let fData = new FormData();
        ...
        <SafeAreaView>
            <ScrollView>
                <TouchableOpacity onPress={() => pickImage()}>
                    <Image source={path ? { uri: path.uri } : { uri: 'http://192.168.137.166:3000/public/uploads/' + item.image }} style={styles.avatar} />
                </TouchableOpacity>
                ...
                <View style={{ flexDirection: "row", marginLeft: 25 }}>
                    <SelectData style={{ marginTop: 15 }} placeholder={item.categoryName} data={categoryList} width={width / 2 - 30} onPress={(item) => setCategory(item._id)} />
                    <SelectData style={{ marginLeft: 10, marginTop: 15 }} placeholder={item.isFeatured == true ? featuredList[0].name : featuredList[1].name} data={featuredList} width={width / 2 - 30} onPress={(item) => setIsFeatured(item._id)} />
                </View>
                <Button label="Cập nhật" onPress={() => _updateProduct(item._id, name, brand, price, description, category, countInStock, rating, numReviews, fImage, isFeatured)} width={width - 50} style={{ alignSelf: "center", marginTop: 30 }} />
